...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
     ...
     */
    //1、对于Spring的默认命名空间http://www.springframework.org/schema/beans下的标签：
    //< import/>、< alias/>、< bean/>、< beans/>，使用parseDefaultElement方法解析；
    //2、对于其他命名空间，比如http://www.springframework.org/schema/context、http://www.springframework.org/schema/aop
    //等等下面的扩展标签：< mvc/>、< task/>、< context/>、< aop/>等，使用parseCustomElement方法解析。
    ...
    protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
        ...
        if (bdHolder != null) {
            /*
             * 2 对于<bean/>标签下的自定义属性和子标签进行解析，基本不会出现自定义属性和子标签的情况，略过
             * 自定义属性就是除了<bean/>标签自带的属性之外的属性，自定义子标签就是非默认命名空间下的标签
             */
            bdHolder = delegate.decorateBeanDefinitionIfRequired(ele, bdHolder);
            try {
                /*
                 * 3 核心实现。将解析之后的bean定义，注册到Register中
                 * 这个Registry实际上就是上下文内部的DefaultListableBeanFactory工厂实例
                 */
            ...
            /*
             * 4 最后发出注册事件，通知相关的监听器，这个Bean已经加载完成了
             * 默认eventListener为EmptyReaderEventListener，它的方法都是空实现，留给子类扩展
             */
