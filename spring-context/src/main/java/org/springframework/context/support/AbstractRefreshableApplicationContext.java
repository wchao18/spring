...
public abstract class AbstractRefreshableApplicationContext extends AbstractApplicationContext {
	...
	protected final void refreshBeanFactory() throws BeansException {
		...
		try {
			...
			//核心方法：加载BeanDefinition,for xml,使用了委派模式
	...
	protected void customizeBeanFactory(DefaultListableBeanFactory beanFactory) {
	    //定义：在registerBeanDefinition注册bean定义的时候判断是否允许同名的BeanDefinition 覆盖
	    //spring默认true
        //springboot中设置了false
		...
		//是否允许循环依赖引用，默认允许true
        //Spring可以自动处理setter方法和注解属性的循环依赖注入，
        //但不能自动处理构造器以及dependsOn的循环依赖注入，并且要求循环依赖的bean不能都是prototye的。
