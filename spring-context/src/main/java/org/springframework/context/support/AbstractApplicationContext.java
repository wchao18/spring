...
import org.springframework.testIgnore.PoJoB;
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
    ...
    protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
        ...
        //不能忽略constructor的自动注入
        //新增逻辑,这里留了一个问题，为什么忽略其中一种一个,对ByName就无效了呢？？？？？？？？？
        beanFactory.ignoreDependencyType(PoJoA.class);
        beanFactory.ignoreDependencyType(PoJoB.class);

        // 修正依赖，这里是注册一些自动装配的特殊规则，比如是BeanFactory class接口的实现类，则在运行时修指定为当前BeanFactory
        //指定接口的具体实现类,一个接口可能存在多个实现类,需要指定好
    ...
    protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
        ...
        //getBeanFactoryPostProcessors() 是addBeanFactoryPostProcessor的
